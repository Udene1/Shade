...
UPDATE inventory_movements m SET unit_cost = p.unit_cost
FROM purchases p WHERE m.type = 'PURCHASE' AND m.reference_id = p.id AND m.unit_cost IS NULL;
UPDATE inventory_movements m SET unit_cost = s.unit_cost
FROM sales s WHERE m.type = 'SALE' AND m.reference_id = s.id AND m.unit_cost IS NULL;

CREATE OR REPLACE FUNCTION shade_fill_inventory_movement_cost()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF NEW.unit_cost IS NULL AND NEW.type = 'PURCHASE' THEN
    SELECT unit_cost INTO NEW.unit_cost FROM purchases WHERE id = NEW.reference_id;
  ELSIF NEW.unit_cost IS NULL AND NEW.type = 'SALE' THEN
    SELECT unit_cost INTO NEW.unit_cost FROM sales WHERE id = NEW.reference_id;
  END IF;
  RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS trg_fill_inventory_movement_cost ON inventory_movements;
CREATE TRIGGER trg_fill_inventory_movement_cost BEFORE INSERT ON inventory_movements
FOR EACH ROW EXECUTE FUNCTION shade_fill_inventory_movement_cost();
...
INSERT INTO inventory_valuation_settings (id, method) VALUES (1, 'WAC') ON CONFLICT (id) DO NOTHING;
...
WITH fifo_layers AS (
  SELECT m.id AS movement_id, m.product_id, m.quantity::numeric AS quantity, COALESCE(m.unit_cost, 0)::numeric AS unit_cost,
    COALESCE(SUM(m.quantity) OVER (PARTITION BY m.product_id ORDER BY m.created_at, m.id ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING), 0)::numeric AS layer_start,
    SUM(m.quantity) OVER (PARTITION BY m.product_id ORDER BY m.created_at, m.id ROWS UNBOUNDED PRECEDING)::numeric AS layer_end
  FROM inventory_movements m WHERE m.quantity > 0
), outflows AS (
  SELECT m.id AS movement_id, m.product_id, m.type,
    COALESCE(SUM(-m.quantity) OVER (PARTITION BY m.product_id ORDER BY m.created_at, m.id ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING), 0)::numeric AS out_start,
    SUM(-m.quantity) OVER (PARTITION BY m.product_id ORDER BY m.created_at, m.id ROWS UNBOUNDED PRECEDING)::numeric AS out_end
  FROM inventory_movements m WHERE m.quantity < 0
), allocations AS (
  ...
  FROM outflows o JOIN fifo_layers l ON l.product_id = o.product_id AND l.layer_start < o.out_end AND l.layer_end > o.out_start
)
...
FROM inventory_valuation_wac_events w LEFT JOIN allocations a ON a.movement_id = w.movement_id
WHERE w.type = 'SALE'
...
WITH fifo_layers AS (
  ...
  SELECT l.product_id, l.movement_id,
    l.quantity - COALESCE(SUM(GREATEST(0::numeric, LEAST(l.layer_end, o.out_end) - GREATEST(l.layer_start, o.out_start))), 0) AS remaining_quantity,
    l.unit_cost
  FROM fifo_layers l LEFT JOIN outflows o ON o.product_id = l.product_id AND l.layer_start < o.out_end AND l.layer_end > o.out_start
  GROUP BY l.product_id, l.movement_id, l.quantity, l.unit_cost, l.layer_start, l.layer_end
), fifo AS (
  SELECT product_id, COALESCE(SUM(remaining_quantity * unit_cost), 0)::numeric AS stock_cost FROM remaining GROUP BY product_id
), wac AS (
...
FROM inventory_valuation v CROSS JOIN inventory_valuation_settings s WHERE s.id = 1;
