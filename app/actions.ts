...
export async function recordCreditPayment(_state: ActionState, formData: FormData): Promise<ActionState> {
  const creditSaleId = positiveInt(text(formData, "credit_sale_id")), amount = nonNegativeMoney(text(formData, "amount")), note = text(formData, "note") || null, op = operationId(formData);
  if (!creditSaleId || amount === null || amount <= 0) return { ok: false, message: "Enter a valid payment amount." };
  try {
    const rows = await sql`WITH op AS (INSERT INTO operation_requests (id, operation_type) VALUES (${op}::uuid, 'CREDIT_PAYMENT') ON CONFLICT (id) DO NOTHING RETURNING id), debt AS (SELECT cs.id, cs.amount_due - COALESCE((SELECT SUM(cp.amount) FROM credit_payments cp WHERE cp.credit_sale_id = cs.id), 0) AS balance FROM credit_sales cs WHERE cs.id = ${creditSaleId} FOR UPDATE), payment AS (INSERT INTO credit_payments (credit_sale_id, amount, note) SELECT id, ${amount}, ${note} FROM debt WHERE ${amount} <= balance AND balance > 0 AND EXISTS (SELECT 1 FROM op) RETURNING id), result AS (SELECT EXISTS(SELECT 1 FROM payment) AS applied) UPDATE operation_requests SET completed_at = NOW(), result = CASE WHEN EXISTS (SELECT 1 FROM result WHERE applied) THEN jsonb_build_object('ok', true, 'message', 'Payment recorded.') ELSE jsonb_build_object('ok', false, 'message', 'Payment exceeds the outstanding balance or the debt was not found.') END WHERE id = ${op}::uuid AND EXISTS (SELECT 1 FROM op) RETURNING result`;
    if (!rows.length) { const prior = await existingOperation(op); if (prior) return prior; return { ok: false, message: "Could not start the payment operation." }; }
    revalidatePath("/"); return rows[0].result as ActionState;
...
export async function closeBusinessDay(_state: ActionState, formData: FormData): Promise<ActionState> {
  ...
  try {
    const rows = await sql`WITH existing AS (SELECT id FROM day_closings WHERE business_date = ${businessDate}::date), sales_day AS (SELECT COALESCE(SUM(s.quantity * s.unit_price) FILTER (WHERE cs.id IS NULL),0)::numeric AS cash_sales, COALESCE(SUM(s.quantity * s.unit_price) FILTER (WHERE cs.id IS NOT NULL),0)::numeric AS credit_sales, COALESCE(SUM(s.quantity),0)::int AS units_sold, COALESCE(SUM(CASE WHEN c.movement_id IS NOT NULL THEN c.wac_cost ELSE s.quantity * s.unit_cost END),0)::numeric AS wac_cost, COALESCE(SUM(CASE WHEN c.movement_id IS NOT NULL THEN c.fifo_cost ELSE s.quantity * s.unit_cost END),0)::numeric AS fifo_cost FROM sales s LEFT JOIN credit_sales cs ON cs.sale_id=s.id LEFT JOIN inventory_movements m ON m.reference_id=s.id AND m.type='SALE' LEFT JOIN inventory_sale_costs c ON c.movement_id=m.id WHERE s.sold_at::date=${businessDate}::date), payments AS (SELECT COALESCE(SUM(amount),0)::numeric AS amount FROM credit_payments WHERE paid_at::date=${businessDate}::date), purchases AS (SELECT COALESCE(SUM(quantity*unit_cost),0)::numeric AS amount FROM purchases WHERE purchased_at::date=${businessDate}::date), debt AS (SELECT COALESCE(SUM(amount_due - COALESCE((SELECT SUM(cp.amount) FROM credit_payments cp WHERE cp.credit_sale_id=cs.id),0)),0)::numeric AS amount FROM credit_sales cs), mismatch AS (SELECT COUNT(*)::int AS count FROM (SELECT p.id FROM products p LEFT JOIN inventory_movements m ON m.product_id=p.id GROUP BY p.id HAVING p.current_stock <> COALESCE(SUM(m.quantity),0)) x), inserted AS (INSERT INTO day_closings (business_date,cash_sales,credit_sales,credit_payments,purchases,sales_revenue,units_sold,wac_gross_profit,fifo_gross_profit,stock_discrepancy_units,outstanding_debt,note) SELECT ${businessDate}::date,s.cash_sales,s.credit_sales,p.amount,pu.amount,s.cash_sales+s.credit_sales,s.units_sold,(s.cash_sales+s.credit_sales)-s.wac_cost,(s.cash_sales+s.credit_sales)-s.fifo_cost,m.count,d.amount,${note} FROM sales_day s CROSS JOIN payments p CROSS JOIN purchases pu CROSS JOIN debt d CROSS JOIN mismatch m WHERE NOT EXISTS (SELECT 1 FROM existing) RETURNING id) SELECT * FROM inserted`;
