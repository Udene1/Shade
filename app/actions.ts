...
async function createSale(productId: number, quantity: number) {
  return sql`
    ...
    FROM sale
    RETURNING product_id, reference_id
  `;
}
...
export async function recordCreditSale(_state: ActionState, formData: FormData): Promise<ActionState> {
  ...
  if (!productId || !quantity || !debtorName) {
    return { ok: false, message: "Select a product, quantity and debtor name." };
  }
  if (!process.env.DATABASE_URL) return { ok: false, message: "Database is not connected yet." };
  ...
  try {
    const rows = await sql`
      WITH product AS (
        ...
        FROM product
        WHERE products.id = product.id AND product.current_stock >= ${quantity}
        RETURNING products.id
      ), sale AS (
        INSERT INTO sales (product_id, quantity, unit_price, unit_cost)
        SELECT product.id, ${quantity}, product.selling_price, product.unit_cost
      ...
      FROM sale
      JOIN credit ON credit.sale_id = sale.id
      RETURNING product_id
    `;
    ...
    return { ok: false, message: "Could not record the credit sale." };
  }
}
