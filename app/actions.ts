...
export async function createProduct(_state: ActionState, formData: FormData): Promise<ActionState> {
  ...
  try {
    ...
    const message = error instanceof Error && error.message.includes("duplicate") ? "A product with that name already exists." : "Could not add the product.";
...
export async function recordPurchase(_state: ActionState, formData: FormData): Promise<ActionState> {
  ...
  try {
    const rows = await sql`
      WITH updated AS (
        UPDATE products SET current_stock = current_stock + ${quantity} WHERE id = ${productId} RETURNING id
      ), purchase AS (
        INSERT INTO purchases (product_id, quantity, unit_cost, supplier) SELECT id, ${quantity}, ${unitCost}, ${supplier} FROM updated RETURNING id, product_id
      )
      INSERT INTO inventory_movements (product_id, type, quantity, reference_id) SELECT product_id, 'PURCHASE', ${quantity}, id FROM purchase RETURNING product_id
    `;
...
async function createSale(productId: number, quantity: number) {
  return sql`
    WITH product AS (
      SELECT id, selling_price, current_stock, COALESCE((SELECT p.unit_cost FROM purchases p WHERE p.product_id = products.id ORDER BY p.purchased_at DESC, p.id DESC LIMIT 1), 0)::numeric AS unit_cost
      FROM products WHERE id = ${productId} FOR UPDATE
    ), updated AS (
      UPDATE products SET current_stock = products.current_stock - ${quantity} FROM product WHERE products.id = product.id AND product.current_stock >= ${quantity} RETURNING products.id
    ), sale AS (
      INSERT INTO sales (product_id, quantity, unit_price, unit_cost) SELECT product.id, ${quantity}, product.selling_price, product.unit_cost FROM product JOIN updated ON updated.id = product.id RETURNING id, product_id
    )
    INSERT INTO inventory_movements (product_id, type, quantity, reference_id) SELECT product_id, 'SALE', -${quantity}, id FROM sale RETURNING product_id, reference_id
  `;
}
  ...
  try {
    const rows = await sql`
      WITH product AS (
        ...
        INSERT INTO sales (product_id, quantity, unit_price, unit_cost) SELECT product.id, ${quantity}, product.selling_price, product.unit_cost FROM product JOIN updated ON updated.id = product.id RETURNING id, product_id, quantity, unit_price
      ), debtor AS (
        INSERT INTO debtors (name, phone, notes)
        SELECT ${debtorName}, ${phone}, ${note}
        WHERE EXISTS (SELECT 1 FROM sale)
        RETURNING id
      ), credit AS (
        INSERT INTO credit_sales (sale_id, debtor_id, amount_due) SELECT sale.id, debtor.id, sale.quantity * sale.unit_price FROM sale CROSS JOIN debtor RETURNING id, sale_id
      )
    ...
    revalidatePath("/");
    return { ok: true, message: `Credit sale recorded for ${debtorName}.` };
...
export async function recordCreditPayment(_state: ActionState, formData: FormData): Promise<ActionState> {
  const creditSaleId = positiveInt(text(formData, "credit_sale_id"));
  const amount = nonNegativeMoney(text(formData, "amount"));
  ...
  try {
    const rows = await sql`
      INSERT INTO credit_payments (credit_sale_id, amount, note)
      SELECT ${creditSaleId}, ${amount}, ${note}
      WHERE ${amount} <= (
        SELECT cs.amount_due - COALESCE((SELECT SUM(cp.amount) FROM credit_payments cp WHERE cp.credit_sale_id = cs.id), 0)
        FROM credit_sales cs WHERE cs.id = ${creditSaleId}
      )
      RETURNING id
    `;
    if (rows.length === 0) return { ok: false, message: "Payment exceeds the outstanding balance or the debt was not found." };
    revalidatePath("/");
