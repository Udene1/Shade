...
export async function recordCreditPayment(_state: ActionState, formData: FormData): Promise<ActionState> {
  ...
  if (!process.env.DATABASE_URL) return { ok: false, message: "Database is not connected yet." };
  try {
    const rows = await sql`INSERT INTO credit_payments (credit_sale_id, amount, note) SELECT ${creditSaleId}, ${amount}, ${note} WHERE ${amount} <= (SELECT cs.amount_due - COALESCE((SELECT SUM(cp.amount) FROM credit_payments cp WHERE cp.credit_sale_id = cs.id), 0) FROM credit_sales cs WHERE cs.id = ${creditSaleId}) RETURNING id`;
